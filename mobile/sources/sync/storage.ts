...
interface StorageState {
    ...
    deepgramStatus: 'disconnected' | 'connecting' | 'connected' | 'error';
    ...
    setDeepgramStatus: (status: 'disconnected' | 'connecting' | 'connected' | 'error') => void;
    ...
    updateSessionModelMode: (sessionId: string, mode: ModelMode) => void;
    // Artifact methods
...
export const storage = create<StorageState>()((set, get) => {
    ...
    return {
                ...
                if (sessionMessages) {
                    ...
                    for (const messageId of changed) {
                        ...
                        if (message.kind === 'user-text') {
                            ...
                            // Skip agent events - not useful for voice
                            continue;
                        }
                    }
        ...
        setDeepgramStatus: (status: 'disconnected' | 'connecting' | 'connected' | 'error') => set((state) => ({
            ...state,
        ...
        updateSessionModelMode: (sessionId: string, mode: ModelMode) => set((state) => {
            const session = state.sessions[sessionId];
            if (!session) return state;
        ...
        assumeUsers: async (userIds: string[]) => {
            // Use the statically imported sync module (imported at top of file)
...
export function useDeepgramStatus(): 'disconnected' | 'connecting' | 'connected' | 'error' {
    return storage(useShallow((state) => state.deepgramStatus));
}
