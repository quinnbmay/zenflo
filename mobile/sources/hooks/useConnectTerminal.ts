...
export function useConnectTerminal(options?: UseConnectTerminalOptions) {
    ...
    const processAuthUrl = React.useCallback(async (url: string) => {
        // Support both zenflo:// (new) and happy:// (legacy) during transition
        if (!url.startsWith('zenflo://terminal?') && !url.startsWith('happy://terminal?')) {
        ...
        setIsLoading(true);
        try {
            const prefix = url.startsWith('zenflo://') ? 'zenflo://terminal?' : 'happy://terminal?';
    ...
    React.useEffect(() => {
        if (CameraView.isModernBarcodeScannerAvailable) {
            const subscription = CameraView.onModernBarcodeScanned(async (event) => {
                if (event.data.startsWith('zenflo://terminal?') || event.data.startsWith('happy://terminal?')) {
