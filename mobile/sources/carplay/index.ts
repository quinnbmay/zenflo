...
import { CarPlay, ListTemplate, GridTemplate } from 'react-native-carplay';
import { Platform } from 'react-native';

// Only initialize on iOS
if (Platform.OS !== 'ios') {
    throw new Error('CarPlay is only available on iOS');
}
...
function setupMainInterface() {
    ...
    mainTemplate.config.onItemSelect = async (e) => {
...
function showTasksList(filter: 'in_progress' | 'todo') {
    ...
    tasksTemplate.config.onItemSelect = async (e) => {
...
function showTaskDetail(taskName: string) {
    ...
    detailTemplate.config.onItemSelect = async (e) => {
        if (e.index === 0) {
...
function showActiveSession() {
    ...
    sessionTemplate.config.onItemSelect = async (e) => {
        if (e.index === 0) {
            activateVoiceAssistant();
        }
    };
