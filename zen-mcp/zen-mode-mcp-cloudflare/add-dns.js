...
async function addDNS() {
  try {
    // Get zone ID for zenflo.dev using curl
    ...
    if (!tokenMatch) {
      ...
      if (oauthMatch) {
        ...
        // Get zone ID
        const zonesCmd = `curl -s -H "Authorization: Bearer ${token}" "https://api.cloudflare.com/client/v4/zones?name=zenflo.dev"`;
        ...
        if (!zones.success || !zones.result || zones.result.length === 0) {
          console.log('Could not find zone for zenflo.dev');
        ...
        if (addResult.success) {
          ...
          console.log('zen.zenflo.dev -> zen-mode-mcp.quinn-4b3.workers.dev');
        } else {
