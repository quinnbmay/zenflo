...
async function daemonPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env.ZENFLO_DAEMON_HTTP_TIMEOUT ? parseInt(process.env.ZENFLO_DAEMON_HTTP_TIMEOUT) : 10_000;
...
export async function isDaemonRunningCurrentlyInstalledHappyVersion(): Promise<boolean> {
  ...
  try {
    ...
    // we will get a new path or not when zenflo is upgraded globally.
